...
# --------------------------------------------------------------------------------------------------
# Archived mode:
#   - exact filtering is based on taxon.conservation_status.status_name
#
# Live mode:
#   - the API query can optionally use a broad threatened flag to widen candidate retrieval
#   - but AFTER retrieval, results are strictly post-filtered using:
#       taxon.conservation_status.status_name
...
# Priority is given to the exact flattened columns the user asked for:
...
# Exact user-facing categories can be restricted to IUCN-style authority where present.
...
make_diagnostics_text <- function(diag) {
  paste0(
    "Query diagnostics:\n",
...
getDeadVertebrates_dateRange <- function(
  ...
  # STRICT post-filtering using taxon.conservation_status.status_name-derived threat_status_std
    ...
    sidebarPanel(
      tabsetPanel(
        ...
        tabPanel(
          ...
          div(
            class = "note-box",
            "Threat-status filtering is strict. Archived mode filters exact status categories using taxon.conservation_status.status_name. Live mode now also post-filters returned records using that same status field, so exact selections like CR are no longer broad threatened matches."
          ),
        ...
        tabPanel(
          ...
          div(
            ...
            tags$ul(
              tags$li("Archived mode filters exact user-selected categories using taxon.conservation_status.status_name."),
        ...
        tabPanel(
          ...
          tags$ul(
            tags$li("Archived mode applies exact filtering directly against standardized status values derived from taxon.conservation_status.status_name."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_query, {
    ...
    if (input$data_source == "live") {
      ...
      if (input$filter_mode == "taxonomy") {
        ...
        # Only widen candidate retrieval where appropriate
  ...
  output$queryDiagnostics <- renderText({
    req(result_data())
    make_diagnostics_text(result_data()$diagnostics)
  })
