...
#   raw_span <- raw_max - raw_min
#   
#   # For manually drawn rectangles, treat as crossing the antimeridian
#   # only if the normalized longitude span is extremely large.
#   # A normal box should have raw_min <= raw_max and a modest span.
#   if (raw_span <= 180) {
#     bbox <- list(
#       swlat = swlat,
#       swlng = raw_min,
...
#     bbox <- list(
#       swlat = swlat,
#       swlng = normalize_longitude(max(lngs360, na.rm = TRUE)),
#       nelat = nelat,
#       nelng = normalize_longitude(min(lngs360, na.rm = TRUE)),
#       crosses_antimeridian = TRUE
#     )
#   }
#   
...
canonicalize_bbox <- function(lats, lngs) {
  ...
  # Treat anything spanning almost the whole globe as near-global
  is_near_global <- effective_span >= 300
...
# Added a very high max_points just in case
# make_leaflet_hotspot_map <- function(df, bbox = NULL, max_points = 5000000) {
...
#     popup_text <- paste0(
#       "<b>", sci_name, "</b><br/>",
#       ifelse(!is.na(obs_date), paste0("Observed on: ", obs_date, "<br/>"), ""),
...
#       )
#     } else {
#       # Conservative fallback for dateline-crossing boxes
#       m <- m %>% fitBounds(
#         lng1 = -180,
#         lat1 = bbox$swlat,
#         lng2 = 180,
...
fetch_dead_data_once <- function(
  ...
  # loc_part <- xglue("&nelat={nelat}&nelng={nelng}&swlat={swlat}&swlng={swlng}")
...
# New fetching total number of observations
...
#   }
#   
#   if (!is.null(taxon_name) && nzchar(taxon_name)) {
...
server <- function(input, output, session) {
  ...
  observeEvent(input$select_map_draw_new_feature, {
    ...
    if (!is.null(feat$geometry) && feat$geometry$type == "Polygon") {
      ...
      bbox <- tryCatch(
        {
          bb <- canonicalize_bbox(lats = lats, lngs = lngs)
          validate_bbox(bb)
          bb
        },
        error = function(e) {
  ...
  observeEvent(input$run_query, {
    ...
    if (input$filter_mode == "taxonomy") {
      ...
      if (input$query_type == "order" && nzchar(input$order_name)) {
        taxon_for_count <- input$order_name
      }
      if (input$query_type == "family" && nzchar(input$family_name)) {
        taxon_for_count <- input$family_name
      }
      if (input$query_type == "taxon" && nzchar(input$taxon_name_input)) {
        taxon_for_count <- input$taxon_name_input
      }
    } else {
