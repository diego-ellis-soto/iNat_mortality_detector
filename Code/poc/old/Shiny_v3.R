...
##################################################################
ui <- fluidPage(
  theme = shinytheme("flatly"),  # <--- A nicer theme from shinythemes
  
  # Title at the top
  titlePanel("Dead Wildlife Observations from iNaturalist"),
  
  # Layout with a sidebar containing TABS: Query, About, Participatory
  sidebarLayout(
    sidebarPanel(
      # Tabs in the sidebar
      tabsetPanel(
        ...
        tabPanel(
          ...
          conditionalPanel(
            ...
            helpText("Draw a rectangle on the map below to define your bounding box."),
        ...
        tabPanel(
          title = "About",
          br(),
          p("This Shiny application queries iNaturalist for observations that have been annotated as Dead wildlife (term_id=17, term_value_id=19). 
            The data is fetched via the iNaturalist API and summarized here for scientific or conservation purposes."),
          p("Use the 'Query' tab to select a place_id or draw a bounding box, choose which years and taxonomic group to include, and then fetch results. 
            The top 90th percentile days are highlighted in the hotspot map and listed in the data table.")
        ),
    ...
    mainPanel(
      # We keep the results in a tabset on the right
      tabsetPanel(
