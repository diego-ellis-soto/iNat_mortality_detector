...
server <- function(input, output, session) {
  ...
  selected_taxonomy_count_query <- reactive({
    ...
    if (input$filter_mode != "taxonomy") {
      return(list(iconic_taxa = NULL, taxon_id = NULL, taxon_name = NULL, label = NULL))
    }
  ...
  output$total_taxon_inat_text <- renderText({
    ...
    if (is.null(taxon_count_query$label) || !nzchar(as.character(taxon_count_query$label)) || is.na(rv$total_taxon_obs_count)) {
      return("")
    }
    
    total_label <- if (!is.null(rv$uploaded_study_area)) {
      "Reference total iNaturalist observations for selected taxon in uploaded-shape bounding box (all observations; separate API count)"
    } else {
      "Reference total iNaturalist observations for selected taxon in selected bounding box (all observations; separate API count)"
    }
  ...
  output$total_taxon_mortality_text <- renderText({
    ...
    if (is.null(taxon_count_query$label) || !nzchar(as.character(taxon_count_query$label)) || is.na(rv$total_taxon_mortality_count)) {
      return("")
    }
    
    total_label <- if (!is.null(rv$uploaded_study_area)) {
      "Reference total mortality observations for selected taxon in uploaded-shape bounding box (mortality-only; separate API count)"
    } else {
      "Reference total mortality observations for selected taxon in selected bounding box (mortality-only; separate API count)"
    }
